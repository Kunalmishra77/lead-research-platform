...
export const researchTasks = app.table(
  'research_tasks',
  {
    ...
    finishedAt: timestamp('finished_at', { withTimezone: true, mode: 'date' }),
    /**
     * Identity within the job (docs/06): type plus a hash of the input. Generated by Postgres,
     * never written, so a writer cannot get its own identity wrong. Unique per job, which makes
     * replanning a no-op instead of a second bill for the same searches (migration 0018).
     */
    taskKey: text('task_key').generatedAlwaysAs(sql`type || ':' || md5(input::text)`),
    createdAt: createdAt(),
    updatedAt: updatedAt(),
  },
  (t) => [
    ...
    uniqueIndex('research_tasks_job_key_uniq').on(t.researchJobId, t.taskKey),
